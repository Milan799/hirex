...
export default function EmployerProfile() {
  ...
  const handleCancel = () => {
    setEditing(false);
    if (user) setForm({ companyName: user.companyName||"", fullName: user.fullName||"", phone: user.phone||"", city: user.location?.city||"", country: user.location?.country||"", website: user.website||"", bio: user.bio||"" });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await dispatch(updateProfile({ companyName: form.companyName, fullName: form.fullName, phone: form.phone, location: { city: form.city, country: form.country }, website: form.website, bio: form.bio })).unwrap();
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="relative mb-6 rounded-3xl overflow-hidden border border-slate-200 dark:border-white/8 transition-colors duration-300">
            ...
            <div className="bg-white px-6 pt-14 pb-5 dark:bg-[#0f0f13] transition-colors duration-300">
              <h2 className="text-xl font-black text-slate-900 dark:text-white transition-colors duration-300">{user?.companyName || "Your Company"}</h2>
